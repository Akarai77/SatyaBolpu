...
const New = ({ type }: { type: NewType }) => {
    ...
    return {
      Details: (
        <Form<DetailsType>
          ...
          submitEndpoint={
            draftable ? `/drafts/${type}/${id}/details` : data.submitApi
          }
          ...
          ? {
              Location: (
                <MAP
                  ...
                  setState={
                    setState as React.Dispatch<
                      React.SetStateAction<typeof state>
                    >
                  }
                  editMode
                />
  ...
  useLayoutEffect(() => {
    ...
    setShowStep(
      Object.keys(steps)[
        Math.min(progress, 100) / (100 / (Object.keys(steps).length - 1))
      ],
    );
  }, [progress]);
  ...
  const userRoutes = ['event', 'location', 'blog'];

  if (
    !authState.token ||
    (authState.user?.role !== 'admin' && !userRoutes.includes(type))
  ) {
    ...
    <div className="w-full mt-20 mb-40 flex flex-col gap-20 items-center justify-center">
      <div className="w-full flex flex-col items-center justify-center gap-10">
        {draftable ? (
          ...
          <Title title={`New ${kebabToTitleCase(type)}`} />
        )}
