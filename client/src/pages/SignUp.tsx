...
const SignUp = () => {
  ...
  const [errors, setErrors] = useState<{ [K in keyof SignUpProps]?: string }>(
    {},
  );
  const [buttonLoad, setButtonLoad] = useState<boolean>(false);
  const [step, setStep] = useState<number>(0);

  const { data, error, loading, post } = useApi('/auth/signup', {
    auto: false,
  });
  ...
  const validateForm = (step: number) => {
    const newErrors: { [K in keyof SignUpProps]?: string } = {};
  ...
  const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    if (formData.image instanceof File) {
      ...
      const res = await uploadApi.post(imageData);
      if (!res) return;
      uploadData.image = res.path;
    }
          ...
          <>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <label htmlFor="image" className="w-fit">
                ...
                justify-center border-3 border-solid border-primary bg-white cursor-pointer"
                >
                  <FaUpload />
                  <p>Upload File</p>
                </div>
