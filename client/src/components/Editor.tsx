...
const Editor = ({
  ...
  const [editorMode, setEditorMode] = useState<'editing' | 'preview'>(
    'editing',
  );
  ...
  const editor = useEditor(
    {
      ...
      content: '',
      onUpdate: ({ editor }) => {
        setFontSize(editor.getAttributes('textStyle').fontSize || 'normal');
      },
    },
    [],
  );
      ...
      >
        ...
        {askEmbedUrl && (
          ...
          >
            ...
            />
            <label htmlFor="url" className="font-black text-[1.5rem]">
              Enter Embed Url
            </label>
            <input
              type="url"
