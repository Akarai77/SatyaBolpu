...
export const validateData = {
  ...
  'post-type': (postType: IPostType) => postType.name,
  'post-group': (postGroup: IPostGroup) => postGroup.name,
...
export const validateForeignFields = {
  post: async (post: IPost) => {
    return (await existsInDB(post.userId, User)) &&
      ...
      post.location
      ? await existsInDB(post.location, Location)
  ...
  blog: async (blog: IBlog) => {
    return (await existsInDB(blog.userId, User)) && blog.location
      ? await existsInDB(blog.location, Location)
      : await Promise.resolve(true);
  },
